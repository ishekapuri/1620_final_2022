...
//id
const cards = `<textarea id ="textArea"rows="30" cols="60"></textarea>`
const displayAllContacts = document.querySelector(".display_all_contacts")


//for (const contacts in contactsList) {
  //displayAllContacts.insertAdjecentHtml("afterbegin", cards)
  //cards.innerText = "cards.name";
//}

function display_contacts() {
  for (const contacts in contactsList) {
    const clickContactCard = document.createElement("button")
    clickContactCard.innerHTML = contacts.name
}
}
